...
<script>
	function setInputEnabled(id) {
	  ...
	  var inputNumber = document.getElementById(id + "Input");
	...
	<body>
		<div class="container">
			...
			<div class="form-group">
				<label class="col-lg-3 control-label">Название вещи</label>
				<div class="col-lg-9">
			...
			<div class="form-group">
				<label class="col-lg-3 control-label">Статус заказа</label>
				<div class="col-lg-9">
			...
			<div class="form-group has-warning">
				<label class="col-lg-3 control-label">Стоимость работы</label>
				<div class="col-lg-9">
			...
			<h3>Таблица запасных частей</h3>
			<c:if test="${order.orderStatus=='REPAIR_PROCESS'}"><h4 class="text-danger">Список деталей уже сформирован</h4></c:if>
			<div class="bs-component">
				...
				<tbody>
					<c:forEach var="part" items="${replacementParst}" varStatus="status">
						<tr id="id${status.index}Row" <c:if test="${part.numberParts!=0}">class="success"</c:if>>
					    	<td><input type="checkbox" name="selectedParts" <c:if test="${order.orderStatus=='REPAIR_PROCESS'}">disabled</c:if> value="${part.partId}" id="id${status.index}" onclick="setInputEnabled(id)" <c:if test="${part.numberParts!=0}">checked</c:if>></td>
					    	<td><c:out value="${status.count}"/></td>
					    	<td><c:out value="${part.partName}"/></td>
					    	<td><c:out value="${part.cost}"/></td>
					    	<td><input type="number" name="numberParts" id="id${status.index}Input" <c:if test="${part.numberParts==0}">disabled</c:if> <c:if test="${order.orderStatus=='REPAIR_PROCESS'}">disabled</c:if> value="${part.numberParts}" min="1" class="form-control"></td>
						</tr>
					</c:forEach>
				</tbody>
				</table>
